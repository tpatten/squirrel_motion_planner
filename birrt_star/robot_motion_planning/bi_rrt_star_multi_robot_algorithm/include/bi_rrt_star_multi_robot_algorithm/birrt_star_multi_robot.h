...
#ifndef BIRRT_STAR_MULTI_ROBOT_H
#define BIRRT_STAR_MULTI_ROBOT_H
...
class BiRRTstarMultiRobotPlanner
{
	public:
    BiRRTstarMultiRobotPlanner(string planning_group_first_robot,string planning_group_second_robot);
    ~BiRRTstarMultiRobotPlanner();

    //Set the planning scene
    void setPlanningSceneInfo(vector<double> size_x, vector<double> size_y, double size_z, string scene_name);
    ...
    void init_planner(char *start_goal_config_file, int search_space);

    //Initialize RRT* Planner (given start and goal config)
    void init_planner(vector<double> start_conf, vector<double> goal_conf, int search_space);

    //Initialize RRT* Planner (given start config and final endeffector pose)
    void init_planner(vector<double> start_conf, vector<double> ee_goal_pose, vector<int> constraint_vec_goal_pose, vector<pair<double,double> > coordinate_dev, int search_space);

    //Initialize RRT* Planner (given start and final endeffector pose)
    void init_planner(vector<double> ee_start_pose, vector<int> constraint_vec_start_pose, vector<double> ee_goal_pose, vector<int> constraint_vec_goal_pose, vector<pair<double,double> > coordinate_dev, int search_space);
    ...
    vector<double> findIKSolution(string tree_name, vector<double> goal_ee_pose, vector<int> constraint_vec, vector<pair<double,double> > coordinate_dev, int max_ik_iter, bool &sol_found , bool show_motion = false);
    //Compute IK for given endeffector goal pose (given a specific initial config)
    vector<double> findIKSolution(string tree_name, vector<double> goal_ee_pose, vector<int> constraint_vec, vector<pair<double,double> > coordinate_dev, vector<double> mean_init_config, int max_ik_iter, bool &sol_found, bool show_motion = false);

    //Attach/Detach an given Object to the End-effector
    void attachObject(moveit_msgs::AttachedCollisionObject attached_object, planning_world::PlanningWorldBuilder *pwb);
    void detachObject(moveit_msgs::AttachedCollisionObject attached_object, planning_world::PlanningWorldBuilder *pwb);
    ...
    void setEdgeCostWeights(vector<double> ecw, int robot_id);
    ...
    vector<vector<double> > getJointTrajectory(string tree_name);
    //Get the planned endeffector trajectory
    vector<vector<double> > getEndeffectorTrajectory(string tree_name);
    ...
    boost::shared_ptr<planning_world::PlanningWorldBuilder> m_planning_world_rob_1;
    boost::shared_ptr<planning_world::PlanningWorldBuilder> m_planning_world_rob_2;
    ...
    //Generate KDL Tree from urdf
    // -> used to generate samples for the kinematic chain of the robot
    boost::shared_ptr<kuka_motion_controller::KDLRobotModel> m_KDLRobotModel_rob_1;
    boost::shared_ptr<kuka_motion_controller::KDLRobotModel> m_KDLRobotModel_rob_2;

    //Create RobotController Object for Motion Control of the robot
    // -> used to connect samples of the RRT* planner
    boost::shared_ptr<kuka_motion_controller::RobotController> m_RobotMotionController_rob_1;
    boost::shared_ptr<kuka_motion_controller::RobotController> m_RobotMotionController_rob_2;

    //FeasibilityChecker to check configurations/nodes and edges for collisions
    boost::shared_ptr<state_feasibility_checker::FeasibilityChecker> m_FeasibilityChecker_rob_1;
    boost::shared_ptr<state_feasibility_checker::FeasibilityChecker> m_FeasibilityChecker_rob_2;
    ...
    //Kinematic Chain of robots
    KDL::Chain m_manipulator_chain_rob_1;
    KDL::Chain m_manipulator_chain_rob_2;
    ...
    vector<double> m_edge_cost_weights_rob_1;
    vector<double> m_edge_cost_weights_rob_2;
    ...
    vector<double> computeEEPose(string tree_name, vector<double> start_conf);
    vector<double> computeEEPose(string tree_name, KDL::JntArray start_conf);
    ...
    KDL::JntArray sampleJointConfigfromEllipse_JntArray(string tree_name);

    //Sample Configuration (as std::vector)
    vector<double> sampleJointConfig_Vector(string tree_name);
    //Sample Configuration (as JntArray)
    KDL::JntArray sampleJointConfig_JntArray(string tree_name);

    //Sample Configuration around mean config vector(as std::vector)
    vector<double> sampleJointConfig_Vector(string tree_name, vector<double> mean_config, double std_dev);
    //Sample Configuration around mean config vector(as JntArray)
    KDL::JntArray sampleJointConfig_JntArray(string tree_name, vector<double> mean_config, double std_dev);
    ...
    bool interpolateConfigurations(string tree_name, Node near_node, Node end_node, int num_points ,vector<vector<double> > &joint_traj, vector<vector<double> > &ee_traj);

    //Compute the cost of an edge connecting two nodes
    vector<double> compute_edge_cost_interpolation(string tree_name, vector<vector<double> > joint_traj);
    ...
    bool stepTowardsRandSample(string tree_name, Node nn_node, Node &x_rand, double extend_step_factor);
    ...
    //Arrays storing the result trajectories for robot 1 and 2
    vector< vector<double> > m_result_joint_trajectory_rob_1;
    vector< vector<double> > m_result_joint_trajectory_rob_2;
    vector< vector<double> > m_result_ee_trajectory_rob_1;
    vector< vector<double> > m_result_ee_trajectory_rob_2;
    ...
    char* m_file_path_joint_trajectory_rob_1;
    char* m_file_path_joint_trajectory_rob_2;
    char* m_file_path_ee_trajectory_rob_1;
    char* m_file_path_ee_trajectory_rob_2;
...
#endif // BiRRT_STAR_MULTI_ROBOT_H
