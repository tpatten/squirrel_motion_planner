...
namespace birrt_star_multi_robot
{

//Constructor
BiRRTstarMultiRobotPlanner::BiRRTstarMultiRobotPlanner(string planning_group_first_robot, string planning_group_second_robot)
{
    //Get namespaces of robots
    string ns_first_robot;
    string ns_second_robot;
    m_nh.param("ns_first_robot", ns_first_robot, std::string("omnirob_group/"));
    m_nh.param("ns_second_robot", ns_second_robot, std::string("robotino_group/"));

    //Read path to folder storing the start and goal configurations from parameter server
    m_nh.param(ns_first_robot + "terminal_configs_path", m_terminal_configs_path, std::string("/home/burgetf/catkin_ws/src/robot_motion_planning/planning_scenarios/Start_Goal_Configurations"));

    //Read package path from parameter server (used for planner statistics files)
    m_nh.param(ns_first_robot + "planner_package_path", m_planner_package_path, std::string("/home/burgetf/catkin_ws/src/robot_motion_planning/birrt_star_algorithm"));

    //Set planning group (needs to correspond to one of the groups defined in the robot srdf)
    m_planning_group = planning_group_first_robot + "_" + planning_group_second_robot;

    //Get names of robot_description parameters
    string robot_description_first_robot;
    string robot_description_second_robot;
    m_nh.param("robot_description_first_robot", robot_description_first_robot, std::string("omnirob_robot_description"));
    m_nh.param("robot_description_second_robot", robot_description_second_robot, std::string("robotino_robot_description"));

    //cout<<robot_description_first_robot<<endl;
    //cout<<robot_description_second_robot<<endl;

    //Planning World for first and second robot (used for collision checks)
    m_planning_world_rob_1 = boost::shared_ptr<planning_world::PlanningWorldBuilder>(new planning_world::PlanningWorldBuilder(robot_description_first_robot, planning_group_first_robot,ns_first_robot));
    m_planning_world_rob_2 = boost::shared_ptr<planning_world::PlanningWorldBuilder>(new planning_world::PlanningWorldBuilder(robot_description_second_robot, planning_group_second_robot,ns_second_robot));

    //TODO
    // -> add planning world builder for second robot
    // -> for robot to robot collision checks enter robots respectively as moveit_msgs:AttachedCollisionObject to planning scene of other robot
    ...
    //Init scene name
    m_planning_world_rob_1->setSceneName("no_name");
    m_planning_world_rob_2->setSceneName("no_name");


    //Create Robot model
    m_KDLRobotModel_rob_1 = boost::shared_ptr<kuka_motion_controller::KDLRobotModel>(new kuka_motion_controller::KDLRobotModel(robot_description_first_robot, ns_first_robot + "planning_scene", ns_first_robot + "endeffector_trajectory", planning_group_first_robot));
    m_KDLRobotModel_rob_2 = boost::shared_ptr<kuka_motion_controller::KDLRobotModel>(new kuka_motion_controller::KDLRobotModel(robot_description_second_robot, ns_second_robot + "planning_scene", ns_second_robot + "endeffector_trajectory", planning_group_second_robot));

    //Create FeasibilityChecker
    m_FeasibilityChecker_rob_1 = boost::shared_ptr<state_feasibility_checker::FeasibilityChecker>(new state_feasibility_checker::FeasibilityChecker(robot_description_first_robot, planning_group_first_robot, ns_first_robot));
    //m_FeasibilityChecker_rob_1->setPlanningSceneServiceName("/omnirob_group/get_planning_scene");
    m_FeasibilityChecker_rob_2 = boost::shared_ptr<state_feasibility_checker::FeasibilityChecker>(new state_feasibility_checker::FeasibilityChecker(robot_description_second_robot, planning_group_second_robot, ns_second_robot));
    //m_FeasibilityChecker_rob_2->setPlanningSceneServiceName("/robotino_group/get_planning_scene");

    //Planning Scene Monitor (required for collision checks)
    //m_planning_scene_monitor =  boost::make_shared<planning_scene_monitor::PlanningSceneMonitor>(robot_description_first_robot);

    //Create Robot Controller
    m_RobotMotionController_rob_1 = boost::shared_ptr<kuka_motion_controller::RobotController>(new kuka_motion_controller::RobotController(robot_description_first_robot, planning_group_first_robot, ns_first_robot));
    //m_RobotMotionController_rob_1->setPlanningSceneServiceName("/omnirob_group/get_planning_scene");
    m_RobotMotionController_rob_2 = boost::shared_ptr<kuka_motion_controller::RobotController>(new kuka_motion_controller::RobotController(robot_description_second_robot, planning_group_second_robot, ns_second_robot));
    //m_RobotMotionController_rob_2->setPlanningSceneServiceName("/robotino_group/get_planning_scene");

    //Set Motion Strategy (0 = all joints weighted equal)
    m_RobotMotionController_rob_1->set_motion_strategy(0);
    m_RobotMotionController_rob_2->set_motion_strategy(0);

    //Get Kinematic Chain of manipulator
    m_manipulator_chain_rob_1 = m_KDLRobotModel_rob_1->getCompleteArmChain();
    m_manipulator_chain_rob_2 = m_KDLRobotModel_rob_2->getCompleteArmChain();

    //Get Name of Base Link (required to set reference frame for visualization)
    m_base_link_name = m_KDLRobotModel_rob_1->getBaseLinkName();

    //Get the joint names
    m_joint_names = m_KDLRobotModel_rob_1->getJointNames();

    //Number of joints
    m_num_joints = m_KDLRobotModel_rob_1->getNumJoints();
    m_num_joints_revolute = m_KDLRobotModel_rob_1->getNumRevoluteJoints();
    m_num_joints_prismatic = m_KDLRobotModel_rob_1->getNumPrismaticJoints();

    //Weights for individual edge cost components (to punish motions of some variables stronger/less than the one of others)
    m_edge_cost_weights_rob_1.resize(m_num_joints);
    m_edge_cost_weights_rob_2.resize(m_KDLRobotModel_rob_2->getNumJoints());

    //Initialize edge cost weights
    for(int j = 0 ; j < m_num_joints ; j++)
    {
        m_edge_cost_weights_rob_1[j] = 1.0;
    }
    for(int j = 0 ; j < m_KDLRobotModel_rob_2->getNumJoints() ; j++)
    {
        m_edge_cost_weights_rob_2[j] = 1.0;
    }
...
//Destructor
BiRRTstarMultiRobotPlanner::~BiRRTstarMultiRobotPlanner()
{
    //Nothing to do yet
}
...
void BiRRTstarMultiRobotPlanner::setPlanningSceneInfo(vector<double> size_x, vector<double> size_y, double size_z, string scene_name)
{
    ...
    //m_planning_world_rob_1->setSceneName(scene_name);
    m_planning_world_rob_1->insertEnvironmentBorders(m_env_size_x,m_env_size_y,size_z,scene_name);
    m_planning_world_rob_2->insertEnvironmentBorders(m_env_size_x,m_env_size_y,size_z,scene_name);
}

void BiRRTstarMultiRobotPlanner::setPlanningSceneInfo(planning_world::PlanningWorldBuilder world_builder)
{
    world_builder.getEnvironmentDimensions(m_env_size_x,m_env_size_y);
    m_planning_world_rob_1->setSceneName(world_builder.getSceneName());
    m_planning_world_rob_2->setSceneName(world_builder.getSceneName());
}

//Initialize RRT* Planner (reading start and goal config from file)
void BiRRTstarMultiRobotPlanner::init_planner(char *start_goal_config_file, int search_space)
{
    ...
    KDL::JntArray start_configuration = m_RobotMotionController_rob_1->Vector_to_JntArray(start_conf);
    KDL::JntArray goal_configuration = m_RobotMotionController_rob_2->Vector_to_JntArray(goal_conf);

    //Find endeffector pose for start and goal configuration
    vector<double> ee_start_pose = computeEEPose("START", start_configuration);
    vector<double> ee_goal_pose = computeEEPose("GOAL", goal_configuration);


    //Extend config vector of rob_2 in order to match config vector length of rob_1 (required to compute "m_cost_theoretical_solution_path")
    vector<double> goal_conf_mod = goal_conf;
    int num_joints_rob_1 = start_conf.size();
    int num_joints_diff = start_conf.size() - goal_conf_mod.size();
    for (int i = 0 ; i < num_joints_diff ; i++)
        goal_conf_mod.push_back(start_conf[num_joints_rob_1 - num_joints_diff + i]);
    ...
    if(m_constraint_active)
    {
        ...
        m_task_frame = m_KDLRobotModel_rob_1->compute_FK_frame(m_manipulator_chain_rob_1, start_configuration);
    }
    ...
    if(search_space == 0) //Planning in Control Space
        ...
        m_cost_theoretical_solution_path = m_Heuristic.euclidean_joint_space_distance(m_manipulator_chain_rob_1, start_conf, goal_conf_mod);
    else
    ...
    m_config_goal_pose = goal_conf_mod;
...
void BiRRTstarMultiRobotPlanner::init_planner(vector<double> start_conf, vector<double> goal_conf, int search_space)
{
...
void BiRRTstarMultiRobotPlanner::init_planner(vector<double> start_conf, vector<double> ee_goal_pose, vector<int> constraint_vec_goal_pose, vector<pair<double,double> > coordinate_dev, int search_space)
{

    //Convert XYZ euler orientation of goal pose to quaternion
    vector<double> quat_goal_pose = m_RobotMotionController_rob_2->convertEulertoQuat(ee_goal_pose[3],ee_goal_pose[4],ee_goal_pose[5]);
    ...
    bool ik_sol_found = false;
    vector<double> ik_sol_ee_goal_pose = findIKSolution("GOAL", goal_ee_pose_quat_orient, constraint_vec_goal_pose, coordinate_dev, start_conf, 100, ik_sol_found, true);
    ...
    //Extend config vector of rob_2 in order to match config vector length of rob_1 (required to compute "m_cost_theoretical_solution_path")
    vector<double> ik_sol_ee_goal_pose_mod = ik_sol_ee_goal_pose;
    int num_joints_rob_1 = start_conf.size();
    int num_joints_diff = start_conf.size() - ik_sol_ee_goal_pose_mod.size();
    for (int i = 0 ; i < num_joints_diff ; i++)
        ik_sol_ee_goal_pose_mod.push_back(start_conf[num_joints_rob_1 - num_joints_diff + i]);
    ...
    if(search_space == 0) //Planning in Control Space
        ...
        m_cost_theoretical_solution_path = m_Heuristic.euclidean_joint_space_distance(m_manipulator_chain_rob_1, start_conf, ik_sol_ee_goal_pose_mod);
    else
        ROS_ERROR("Requested planner search space does not exist!!!");
    ...
    //Save start and goal config
    m_config_start_pose = start_conf;
    m_config_goal_pose = ik_sol_ee_goal_pose_mod;
...
void BiRRTstarMultiRobotPlanner::init_planner(vector<double> ee_start_pose, vector<int> constraint_vec_start_pose, vector<double> ee_goal_pose, vector<int> constraint_vec_goal_pose, vector<pair<double,double> > coordinate_dev, int search_space)
{

    //Convert XYZ euler orientation of start pose to quaternion
    vector<double> quat_start_pose = m_RobotMotionController_rob_1->convertEulertoQuat(ee_start_pose[3],ee_start_pose[4],ee_start_pose[5]);
    ...
    bool goal_ik_sol_found = false;
    vector<double> ik_sol_ee_goal_pose = findIKSolution("GOAL",goal_ee_pose_quat_orient, constraint_vec_goal_pose, coordinate_dev, 100, goal_ik_sol_found, true);

    cout<<"Goal config generated successfully"<<endl;
    ...
    bool start_ik_sol_found = false;
    vector<double> ik_sol_ee_start_pose = findIKSolution("START", start_ee_pose_quat_orient, constraint_vec_start_pose, coordinate_dev, 100, start_ik_sol_found, true);

     cout<<"Start config generated successfully"<<endl;
//    for(int i = 0 ; i < ik_sol_ee_goal_pose.size() ; i++)
//        cout<<ik_sol_ee_goal_pose[i]<<" ";

//    cout<<endl;

//    for(int i = 0 ; i < ik_sol_ee_start_pose.size() ; i++)
//        cout<<ik_sol_ee_start_pose[i]<<" ";

//    cout<<endl;

    //Extend config vector of rob_2 in order to match config vector length of rob_1 (required to compute "m_cost_theoretical_solution_path")
    vector<double> ik_sol_ee_goal_pose_mod = ik_sol_ee_goal_pose;
    int num_joints_rob_1 = ik_sol_ee_start_pose.size();
    int num_joints_diff = ik_sol_ee_start_pose.size() - ik_sol_ee_goal_pose_mod.size();
    for (int i = 0 ; i < num_joints_diff ; i++)
        ik_sol_ee_goal_pose_mod.push_back(ik_sol_ee_start_pose[num_joints_rob_1 - num_joints_diff + i]);
    ...
    if(search_space == 0) //Planning in Control Space
        ...
        m_cost_theoretical_solution_path = m_Heuristic.euclidean_joint_space_distance(m_manipulator_chain_rob_1, ik_sol_ee_start_pose, ik_sol_ee_goal_pose_mod);
    else
        ROS_ERROR("Requested planner search space does not exist!!!");
...
vector<double> BiRRTstarMultiRobotPlanner::computeEEPose(string tree_name, vector<double> start_conf)
{

    //Robot controller, model to be used
    boost::shared_ptr<kuka_motion_controller::RobotController> controller;
    boost::shared_ptr<kuka_motion_controller::KDLRobotModel> robot_model;
    //Kinematic chain to be used
    KDL::Chain kinematic_chain;

    if(tree_name == "START")
    {
        controller = m_RobotMotionController_rob_1;
        robot_model = m_KDLRobotModel_rob_1;
        kinematic_chain = m_manipulator_chain_rob_1;
    }
    else if(tree_name == "GOAL")
    {
        controller = m_RobotMotionController_rob_2;
        robot_model = m_KDLRobotModel_rob_2;
        kinematic_chain = m_manipulator_chain_rob_2;
    }
    else
    {
        ROS_ERROR("tree not known!");
    }

    //Convert start configuration into KDL Joint Array
    KDL::JntArray start_configuration = controller->Vector_to_JntArray(start_conf);
...
//        start_configuration(i) = start_conf[i];
    ...
    vector<double> ee_start_pose = robot_model->compute_FK(kinematic_chain, start_configuration);
...
//Compute pose of endeffector in start configuration
vector<double> BiRRTstarMultiRobotPlanner::computeEEPose(string tree_name, KDL::JntArray start_conf)
{
    //Robot model to be used
    boost::shared_ptr<kuka_motion_controller::KDLRobotModel> robot_model;
    //Kinematic chain to be used
    KDL::Chain kinematic_chain;
    ...
    //Compute endeffector pose for start node
    vector<double> ee_start_pose = robot_model->compute_FK(kinematic_chain, start_conf);
...
vector<double> BiRRTstarMultiRobotPlanner::findIKSolution(string tree_name, vector<double> goal_ee_pose, vector<int> constraint_vec, vector<pair<double,double> > coordinate_dev, int max_ik_iter, bool &sol_found, bool show_motion)
{

    //Robot controller, model and Feasibility checker to be used
    boost::shared_ptr<kuka_motion_controller::RobotController> controller;
    boost::shared_ptr<state_feasibility_checker::FeasibilityChecker> feasibility_checker;

    if(tree_name == "START")
    {
        controller = m_RobotMotionController_rob_1;
        feasibility_checker = m_FeasibilityChecker_rob_1;
    }
    else if(tree_name == "GOAL")
    {
        controller = m_RobotMotionController_rob_2;
        feasibility_checker = m_FeasibilityChecker_rob_2;
    }
    else
    {
        ROS_ERROR("tree not known!");
    ...
    controller->setVariableConstraints(constraint_vec, coordinate_dev);


    //Run numerical IK Solver
    vector<double> ik_solution;
    while (collision_free == false && max_ik_iter != 0)
    {
        ...
        vector<double> conf_init = sampleJointConfig_Vector(tree_name);

        //Set new random start config
        controller->setStartConf(conf_init);
        ...
        controller->set_EE_goal_pose(goal_ee_pose);

        //control loop execution
        kuka_motion_controller::Status connector_state = controller->run_VDLS_Control_Connector(1000, joint_trajectory,ee_trajectory,show_motion,show_motion);
        ...
        if(connector_state == kuka_motion_controller::REACHED)
        {
            ...
            if(feasibility_checker->isConfigValid(ik_solution))
            {
                collision_free = true;
                sol_found = true;
            }
        ...
        //Decrement ik run counter
        max_ik_iter--;

        //Leave loop when maximum number of iterations have been reached
        if(max_ik_iter == 0)
            break;
...
vector<double> BiRRTstarMultiRobotPlanner::findIKSolution(string tree_name, vector<double> goal_ee_pose, vector<int> constraint_vec, vector<pair<double,double> > coordinate_dev, vector<double> mean_init_config, int max_ik_iter, bool &sol_found, bool show_motion)
{
    ...
    while (collision_free == false && max_ik_iter != 0)
    {
        ...
        //Get  random start config
        vector<double> conf_init = sampleJointConfig_Vector(tree_name, mean_init_config, sampling_stddev);
...
//        }
        ...
        if(connector_state == kuka_motion_controller::REACHED)
        {
            cout<<"Start EE Pose reached ...."<<endl;
        ...
        {
            cout<<"Failed to reach Start EE Pose!"<<endl;
        }
...
vector<vector<double> > BiRRTstarMultiRobotPlanner::generate_start_goal_config(vector<double> start_pose, vector<int> constraint_vec_start_pose, vector<double> goal_pose, vector<int> constraint_vec_goal_pose, vector<pair<double,double> > coordinate_dev, bool show_motion )
{
    ...
    string folder_path = m_terminal_configs_path + "/Start_Goal_Configurations/"+ m_planning_group + "_" + m_planning_world_rob_1->getSceneName() + "_start_goal_config.txt";
    ...
    vector<double> quat_start_pose = m_RobotMotionController_rob_1->convertEulertoQuat(start_pose[3],start_pose[4],start_pose[5]);
    vector<double> quat_goal_pose = m_RobotMotionController_rob_1->convertEulertoQuat(goal_pose[3],goal_pose[4],goal_pose[5]);
    ...
    //      -> Initial values for prismatic joint are sampled uniformly from their respective joint range)
    bool goal_ik_sol_found = false;
    start_goal_config[1] = findIKSolution("GOAL", goal_ee_pose_quat_orient, constraint_vec_goal_pose, coordinate_dev, 100, goal_ik_sol_found, show_motion);
    ...
    bool start_ik_sol_found = false;
    start_goal_config[0] = findIKSolution("START",start_ee_pose_quat_orient, constraint_vec_start_pose, coordinate_dev, 100, start_ik_sol_found, show_motion);
...
void BiRRTstarMultiRobotPlanner::activateTreeOptimization()
{
    m_tree_optimization_active = true;
}

//Deactivate Tree Optimization Features
void BiRRTstarMultiRobotPlanner::deactivateTreeOptimization()
{
...
void BiRRTstarMultiRobotPlanner::activateInformedSampling()
{
    m_informed_sampling_active = true;
}

//Deactivate Informed Sampling Heuristic Features
void BiRRTstarMultiRobotPlanner::deactivateInformedSampling()
{
...
bool BiRRTstarMultiRobotPlanner::run_planner(int search_space, bool flag_iter_or_time, double max_iter_time, bool show_tree_vis, double iter_sleep, int planner_run_number)
{

    cout<<"Planner started successfully"<<endl;
    ...
    string scenario_name = m_planning_world_rob_1->getSceneName();
    ...
    //Set path to the file that will store the planned joint trajectory for robot 1
    string folder_path = m_planner_package_path + "/data/"+ m_planner_type +"/"+  scenario_name  + "_joint_trajectory_rob_1_run_" + string_planner_run_number +".txt";
    m_file_path_joint_trajectory_rob_1 = new char[folder_path.size() + 1];
    copy(folder_path.begin(), folder_path.end(), m_file_path_joint_trajectory_rob_1);
    m_file_path_joint_trajectory_rob_1[folder_path.size()] = '\0'; // don't forget the terminating 0
    //cout<<m_file_path_joint_trajectory_rob_1<<endl;

    //Set path to the file that will store the planned joint trajectory for robot 2
    folder_path = m_planner_package_path + "/data/"+ m_planner_type +"/"+  scenario_name  + "_joint_trajectory_rob_2_run_" + string_planner_run_number +".txt";
    m_file_path_joint_trajectory_rob_2 = new char[folder_path.size() + 1];
    copy(folder_path.begin(), folder_path.end(), m_file_path_joint_trajectory_rob_2);
    m_file_path_joint_trajectory_rob_2[folder_path.size()] = '\0'; // don't forget the terminating 0
    //cout<<m_file_path_joint_trajectory_rob_2<<endl;

    //Set path to the file that will store the planned ee trajectory for robot 1
    folder_path = m_planner_package_path + "/data/"+ m_planner_type +"/"+  scenario_name  + "_ee_trajectory_rob_1_run_" + string_planner_run_number +".txt";
    m_file_path_ee_trajectory_rob_1 = new char[folder_path.size() + 1];
    copy(folder_path.begin(), folder_path.end(), m_file_path_ee_trajectory_rob_1);
    m_file_path_ee_trajectory_rob_1[folder_path.size()] = '\0'; // don't forget the terminating 0
    //cout<<m_file_path_ee_trajectory_rob_1<<endl;

    //Set path to the file that will store the planned ee trajectory for robot 2
    folder_path = m_planner_package_path + "/data/"+ m_planner_type +"/"+  scenario_name  + "_ee_trajectory_rob_2_run_" + string_planner_run_number +".txt";
    m_file_path_ee_trajectory_rob_2 = new char[folder_path.size() + 1];
    copy(folder_path.begin(), folder_path.end(), m_file_path_ee_trajectory_rob_2);
    m_file_path_ee_trajectory_rob_2[folder_path.size()] = '\0'; // don't forget the terminating 0
    //cout<<m_file_path_ee_trajectory_rob_2<<endl;
    ...
    //Permitted axes of displacement w.r.t. desired hand over ee pose
    vector<int> constraint_vec_inter_rob_pose(6); // (0 = don't care, 1 = constraint)
    constraint_vec_inter_rob_pose[0] = 1; //X
    constraint_vec_inter_rob_pose[1] = 1; //Y
    constraint_vec_inter_rob_pose[2] = 1; //Z
    constraint_vec_inter_rob_pose[3] = 1; //RotX
    constraint_vec_inter_rob_pose[4] = 1; //RotY
    constraint_vec_inter_rob_pose[5] = 1; //RotZ

    //Permitted displacement of ee coordinates w.r.t desired hand over ee pose
    vector<pair<double,double> > inter_rob_pose_coordinate_dev(6);
    inter_rob_pose_coordinate_dev[0].first = -0.005;    //negative X deviation [m]
    inter_rob_pose_coordinate_dev[0].second = 0.005;    //positive X deviation
    inter_rob_pose_coordinate_dev[1].first = -0.005;    //negative Y deviation
    inter_rob_pose_coordinate_dev[1].second = 0.005;    //positive Y deviation
    inter_rob_pose_coordinate_dev[2].first = -0.005;    //negative Z deviation
    inter_rob_pose_coordinate_dev[2].second = 0.005;    //positive Z deviation
    inter_rob_pose_coordinate_dev[3].first = -0.05;     //negative Xrot deviation [rad]
    inter_rob_pose_coordinate_dev[3].second = 0.05;     //positive Xrot deviation
    inter_rob_pose_coordinate_dev[4].first = -0.05;     //negative Yrot deviation
    inter_rob_pose_coordinate_dev[4].second = 0.05;     //positive Yrot deviation
    inter_rob_pose_coordinate_dev[5].first = -0.05;     //negative Zrot deviation
    inter_rob_pose_coordinate_dev[5].second = 0.05;     //positive Zrot deviation
    ...
    while((flag_iter_or_time == 0 && m_executed_planner_iter < m_max_planner_iter) || (flag_iter_or_time == 1 && m_executed_planner_time < m_max_planner_time))
    {
        ...
        if(search_space == 1)
        {
            ...
            {
                config_rand = sampleJointConfigfromEllipse_JntArray(tree_A->name);

            }
            else
            {
                //Sample Joint configuration uniformly
                config_rand = sampleJointConfig_JntArray(tree_A->name);
            }


            //Set Data for random new Node
            x_rand.node_id = tree_A->nodes.size();
            if(tree_A->name == "START")
            {
                x_rand.config = m_RobotMotionController_rob_1->JntArray_to_Vector(config_rand);
                x_rand.ee_pose = m_KDLRobotModel_rob_1->compute_FK(m_manipulator_chain_rob_1,config_rand);
            }
            else if(tree_A->name == "GOAL")
            {
                x_rand.config = m_RobotMotionController_rob_2->JntArray_to_Vector(config_rand);
                x_rand.ee_pose = m_KDLRobotModel_rob_2->compute_FK(m_manipulator_chain_rob_2,config_rand);
            }
            else
                ROS_ERROR("Tree not known!");
            ...
            if(tree_extend_NN == true || tree_extend_BP == true)
            {
                ...
                // ----------------- START: HIGHLY EXPERIMENTAL ---------------------


                //To add the other robot as a collision object
                moveit_msgs::AttachedCollisionObject robot_base_coll_object;
                //Position
                vector<double> pos_other_rob(2);
                pos_other_rob[0] = x_new.config[0]; //x position
                pos_other_rob[1] = x_new.config[1]; //y position
                //Orientation (Rotation around z-axis as Quaternion)
                vector<double> orient_other_rob(4);
                orient_other_rob[0] = 0.0; //quat x
                orient_other_rob[1] = 0.0; //quat y
                orient_other_rob[2] = sin( x_new.config[3] / 2.0 ); //quat z
                orient_other_rob[3] = cos( x_new.config[3] / 2.0 ); //quat w


                //Find ee pose for new config
                KDL::Frame ee_pose_new_node;
                //Config of x_new as JntArray (either from rob_1 or rob_2)
                KDL::JntArray jnt_array = m_RobotMotionController_rob_1->Vector_to_JntArray(x_new.config);
                //Relative transformation from endeffector of one robot to the endeffector of the other robot
                KDL::Vector inter_trans_robots = KDL::Vector(0.0,0.0,0.2); //20cm offset in z-direction
                KDL::Rotation inter_rot_robots = KDL::Rotation(1,0,0,0,-1,0,0,0,-1);
                KDL::Frame inter_ee_transform_robots(inter_rot_robots,inter_trans_robots);
                ...
                if(tree_A->name == "START")
                {
                    //Insert Base of first robot as collision object for second robot
                    robot_base_coll_object = m_planning_world_rob_2->insertRobotBase(pos_other_rob, orient_other_rob, "omnirob");

                    //Get EE pose of rob_1
                    ee_pose_new_node = m_KDLRobotModel_rob_1->compute_FK_frame(m_manipulator_chain_rob_1, jnt_array);

                    //Detemine required EE pose for rob_2
                    KDL::Frame desired_ee_pose_rob_2 = ee_pose_new_node * inter_ee_transform_robots;

                    //Set up goal pose with orientation expressed by quaternion
                    vector<double> desired_ee_pose_rob_2_quat_orient (7);
                    desired_ee_pose_rob_2_quat_orient[0] = desired_ee_pose_rob_2.p.x(); //x
                    desired_ee_pose_rob_2_quat_orient[1] = desired_ee_pose_rob_2.p.y(); //y
                    desired_ee_pose_rob_2_quat_orient[2] = desired_ee_pose_rob_2.p.z(); //z
                    desired_ee_pose_rob_2.M.GetQuaternion(desired_ee_pose_rob_2_quat_orient[3],desired_ee_pose_rob_2_quat_orient[4],desired_ee_pose_rob_2_quat_orient[5],desired_ee_pose_rob_2_quat_orient[6]);

                    //Find IK solution for required rob_2 ee pose
                    bool ik_sol_found = false;
                    vector<double> ik_sol_rob_2 = findIKSolution("GOAL", desired_ee_pose_rob_2_quat_orient, constraint_vec_inter_rob_pose, inter_rob_pose_coordinate_dev, 1, ik_sol_found, false);

                    //Delete Base of second robot as collision object for first robot
                    m_planning_world_rob_2->deleteCollisionObject(robot_base_coll_object);

                    if(ik_sol_found)
                    {
                        //Generate x_new for rob_2
                        Node x_new_rob_2;
                        x_new_rob_2.config = ik_sol_rob_2;
                        x_new_rob_2.node_id = x_new.node_id;
                        x_new_rob_2.parent_id = x_new.parent_id; //??????
                        x_new_rob_2.cost_reach.total = x_new.cost_reach.total;
                        x_new_rob_2.cost_reach.prismatic = x_new.cost_reach.prismatic;
                        x_new_rob_2.cost_reach.revolute = x_new.cost_reach.revolute;

                        //Search for nearest neighbour to x_new in tree "tree_B"
                        Node x_connect = find_nearest_neighbour_interpolation(tree_B, x_new_rob_2.config);

                        //Try to connect the two RRT* Trees given the new node added to "tree_A" and its nearest neighbour in "tree_B"
                        connectGraphsInterpolation(tree_B, x_connect, x_new_rob_2, show_tree_vis);

                        //cout<<"Tried to connect graphs!"<<endl;
                    }
                    else
                    {
                        //cout<<"Failed to generate config for other robot!"<<endl;
                    }
                ...
                {    
                    ...
                    //Insert Base of second robot as collision object for first robot
                    robot_base_coll_object = m_planning_world_rob_1->insertRobotBase(pos_other_rob, orient_other_rob, "robotino");

                    //Get EE pose of rob_2
                    ee_pose_new_node = m_KDLRobotModel_rob_2->compute_FK_frame(m_manipulator_chain_rob_2, jnt_array);

                    //Detemine required EE pose for rob_1
                    KDL::Frame desired_ee_pose_rob_1 = ee_pose_new_node * inter_ee_transform_robots;

                    //Set up goal pose with orientation expressed by quaternion
                    vector<double> desired_ee_pose_rob_1_quat_orient (7);
                    desired_ee_pose_rob_1_quat_orient[0] = desired_ee_pose_rob_1.p.x(); //x
                    desired_ee_pose_rob_1_quat_orient[1] = desired_ee_pose_rob_1.p.y(); //y
                    desired_ee_pose_rob_1_quat_orient[2] = desired_ee_pose_rob_1.p.z(); //z
                    desired_ee_pose_rob_1.M.GetQuaternion(desired_ee_pose_rob_1_quat_orient[3],desired_ee_pose_rob_1_quat_orient[4],desired_ee_pose_rob_1_quat_orient[5],desired_ee_pose_rob_1_quat_orient[6]);

                    //Find IK solution for required rob_1 ee pose
                    bool ik_sol_found = false;
                    vector<double> ik_sol_rob_1 = findIKSolution("START", desired_ee_pose_rob_1_quat_orient, constraint_vec_inter_rob_pose, inter_rob_pose_coordinate_dev, 1, ik_sol_found, false);

                    //Delete Base of first robot as collision object for second robot
                    m_planning_world_rob_1->deleteCollisionObject(robot_base_coll_object);

                    if(ik_sol_found)
                    {
                        //Generate x_new for rob_2
                        Node x_new_rob_1;
                        x_new_rob_1.config = ik_sol_rob_1;
                        x_new_rob_1.node_id = x_new.node_id;
                        x_new_rob_1.parent_id = x_new.parent_id; //??????
                        x_new_rob_1.cost_reach.total = x_new.cost_reach.total;
                        x_new_rob_1.cost_reach.prismatic = x_new.cost_reach.prismatic;
                        x_new_rob_1.cost_reach.revolute = x_new.cost_reach.revolute;


                        //Search for nearest neighbour to x_new in tree "tree_B"
                        Node x_connect = find_nearest_neighbour_interpolation(tree_B, x_new_rob_1.config);

                        //Try to connect the two RRT* Trees given the new node added to "tree_A" and its nearest neighbour in "tree_B"
                        connectGraphsInterpolation(tree_B, x_connect, x_new_rob_1, show_tree_vis);
       ...
       //cin>>test;
        ...
        if(tree_idx == true)
        {
            ...
            //chain_tree_A = &m_manipulator_chain_rob_2;
            //chain_tree_B = &m_manipulator_chain_rob_1;
            //cout<<"Respective Kinematic Chains swapped 1"<<endl;
        }
        else
        {
            tree_A = &m_start_tree;
            ...
            //chain_tree_A = &m_manipulator_chain_rob_1;
            //chain_tree_B = &m_manipulator_chain_rob_2;
            //cout<<"Respective Kinematic Chains swapped 2"<<endl;
        }


        //Switch order of respective controllers
        //std::reverse(controller.begin(),controller.end());
...
vector<vector<double> > BiRRTstarMultiRobotPlanner::getJointTrajectory(string tree_name)
{
    if(tree_name == "START")
        return m_result_joint_trajectory_rob_1;
    else if(tree_name == "GOAL")
        return m_result_joint_trajectory_rob_2;
    else
    {
        ROS_ERROR("Tree not known!");
    }
...
vector<vector<double> > BiRRTstarMultiRobotPlanner::getEndeffectorTrajectory(string tree_name)
{
    if(tree_name == "START")
        return m_result_ee_trajectory_rob_1;
    else if(tree_name == "GOAL")
        return m_result_ee_trajectory_rob_2;
    else
    {
        ROS_ERROR("Tree not known!");
    }
...
void BiRRTstarMultiRobotPlanner::reset_planner()
{
    ...
    //Init total number if nodes and edges contained in the goal tree
    ...
    m_result_joint_trajectory_rob_1.clear();
    m_result_joint_trajectory_rob_2.clear();
    m_result_ee_trajectory_rob_1.clear();
    m_result_ee_trajectory_rob_2.clear();
...
bool BiRRTstarMultiRobotPlanner::expandTree(Rrt_star_tree *tree, Node nn_node, Node x_rand, Node &x_new, Edge &e_new, bool show_tree_vis)
{
    ...
    if(tree->name == "START")
    {
        ...
        //Number of joints
        m_num_joints = robot_model->getNumJoints();
        m_num_joints_revolute = robot_model->getNumRevoluteJoints();
        m_num_joints_prismatic = robot_model->getNumPrismaticJoints();
    }
    else if(tree->name == "GOAL")
    {
        controller = m_RobotMotionController_rob_2;
        robot_model = m_KDLRobotModel_rob_2;
        kinematic_chain = m_manipulator_chain_rob_2;
    ...
    if(m_constraint_active)
    {
        ...
        bool projection_x_rand = controller->run_config_retraction(x_rand.config, m_task_frame, m_grasp_transform, m_constraint_vector, m_coordinate_dev, m_max_projection_iter);
        ...
        while(rand_sample_reached == false)
        {
            ...
            rand_sample_reached = stepTowardsRandSample(tree->name, nn_node, exp_node_towards_rand_node, m_constraint_extend_step_factor);
            ...
            if(rand_sample_reached == false)
            {
                ...
                bool projection_succeed = controller->run_config_retraction(exp_node_towards_rand_node.config, m_task_frame, m_grasp_transform, m_constraint_vector, m_coordinate_dev, m_max_projection_iter);

                if(projection_succeed == true && feasibility_checker->isConfigValid(exp_node_towards_rand_node.config))
                {
                    ...
                    if(connection_result == true && feasibility_checker->isEdgeValid(gen_edge))
                    {
                        //Modify node data
...
//        //Project sample onto constraint manifold
//        bool projection_succeed = controller->run_config_retraction(x_rand.config, m_task_frame, m_grasp_transform, m_constraint_vector, m_max_projection_iter, m_projection_error_threshold);

//        if(projection_succeed == true && feasibility_checker->isConfigValid(x_rand.config))
//        {
...
//            if(feasibility_checker->isConfigValid(x_rand.config))
//            {
//                KDL::JntArray projected_sample = controller->Vector_to_JntArray(x_rand.config);
//                x_rand.ee_pose = robot_model->compute_FK(kinematic_chain,projected_sample);
//            }
//            else
//            {
...
//        if(connection_result == true && feasibility_checker->isEdgeValid(e_new))
//        {
...
//        }
    ...
    else //Unconstraint motion planning
    {
        ...
        feasibility_checker->isEdgeValid(e_new, index_last_valid_node);
        ...
        if(index_last_valid_node != 0)
        {
            ...
            while(rand_sample_reached == false)
            {
                ...
                rand_sample_reached = stepTowardsRandSample(tree->name, nn_node, original_extend_node, m_unconstraint_extend_step_factor);
...
//Try to connect the two RRT* Trees given the new node added to "tree_A" and its nearest neighbour in "tree_B"
void BiRRTstarMultiRobotPlanner::connectGraphsControl(Rrt_star_tree *tree, Node x_connect, Node x_new, bool show_tree_vis)
{
    ...
    if(connector_result == kuka_motion_controller::REACHED)
    {
        ...
        {
            if(m_FeasibilityChecker_rob_1->isEdgeValid(gen_edge))
            {
            ...
            else
            {
                ...
                for (int i = 1 ; i < gen_edge.ee_trajectory.size()-1 ; i+=extend_step_width)
                {
                    ...
                    {
                        if(m_FeasibilityChecker_rob_1->isEdgeValid(ext_edge))
                        {
                            //Set Node ID
...
//Try to connect the two RRT* Trees given the new node added to "tree_A" and its nearest neighbour in "tree_B"
void BiRRTstarMultiRobotPlanner::connectGraphsInterpolation(Rrt_star_tree *tree, Node x_connect, Node x_new, bool show_tree_vis)
{
...
//    double time_start = m_timer.tv_sec+(m_timer.tv_usec/1000000.0);
    ...
    //Set the repsective controller and feasibility checker
    if(tree->name == "START")
    {
        controller = m_RobotMotionController_rob_1;
        feasibility_checker = m_FeasibilityChecker_rob_1;
    ...
    if(m_constraint_active == false)
    {
        ...
        if(solution_cost[0] < cost_solution_path[0])
        {
            ...
            if(feasibility_checker->isEdgeValid(gen_edge, index_last_valid_node))
            {
                //Set solution path cost (will be probably improved by on of the near nodes, see code below)
                ...
                while(rand_sample_reached == false)
                {
                    ...
                    rand_sample_reached = stepTowardsRandSample(tree->name, x_connect, original_x_new, m_unconstraint_extend_step_factor);
            ...
            else
            {
                ...
                if(index_last_valid_node != 0)
                {
                    ...
                    while(rand_sample_reached == false)
                    {
                        //Perform a step from current x_connect towards q_rand using a fixed step size (returns true when random sample has been reached by expansion)
                        Node original_extend_node = extend_node;
                        rand_sample_reached = stepTowardsRandSample(tree->name, x_connect, original_extend_node, m_unconstraint_extend_step_factor);
                   ...
                   if(solution_cost[0] < cost_solution_path[0])
                    {
                        ...
                        int index_last_valid_node = 0;
                        //Only if edge to x_new is valid
                        if(feasibility_checker->isEdgeValid(ext_edge, index_last_valid_node))
                        {
//                            cout<<tree->name<<endl;
...
//                            cin>>t;
                            ...
                            while(rand_sample_reached == false)
                            {
                                ...
                                Node original_x_new = x_new;
                                rand_sample_reached = stepTowardsRandSample(tree->name, curr_near_node, original_x_new, m_unconstraint_extend_step_factor);
                            ...
                            if (cost_solution_path[0] == m_cost_best_solution_path && solution_cost[0] < best_near_vertex_sol_cost)
                            {
                                ...
                                if(index_last_valid_node != 0)
                                {
                                    ...
                                    while(rand_sample_reached == false)
                                    {
                                        ...
                                        Node original_extend_node = extend_node;
                                        rand_sample_reached = stepTowardsRandSample(tree->name, curr_near_node, original_extend_node, m_unconstraint_extend_step_factor);
    ...
    else
    {
        ...
        while(rand_sample_reached == false)
        {
            ...
            rand_sample_reached = stepTowardsRandSample(tree->name, x_connect, exp_node_towards_tree_A, m_constraint_extend_step_factor);
            ...
            if(rand_sample_reached == false)
            {
                ...
                //Project sample onto constraint manifold
                bool projection_succeed = controller->run_config_retraction(exp_node_towards_tree_A.config, m_task_frame, m_grasp_transform, m_constraint_vector, m_coordinate_dev, m_max_projection_iter);

                if(projection_succeed == true && feasibility_checker->isConfigValid(exp_node_towards_tree_A.config))
                {
                    ...
                    if(connection_result == true && feasibility_checker->isEdgeValid(gen_edge))
                    {
                        //KDL::JntArray projected_sample = controller->Vector_to_JntArray(exp_node_towards_tree_A.config);
                        //exp_node_towards_tree_A.ee_pose = robot_model->compute_FK(kinematic_chain,projected_sample);
...
//Insert Node and Edge into the RRT* Tree
void BiRRTstarMultiRobotPlanner::insertNode(Rrt_star_tree *tree, Edge e_new, Node x_new, bool show_tree_vis)
{
    //Assign ID to edge
...
//bool BiRRTstarMultiRobotPlanner::solution_path_available()
//{
...
vector<double> BiRRTstarMultiRobotPlanner::sampleEEpose()
{
    //Get Random Pose for the endeffector
    vector<double> sampled_ee_pose = m_RobotMotionController_rob_1->getRandEEPose();
...
vector<double> BiRRTstarMultiRobotPlanner::sampleEEposefromEllipse()
{
    ...
    while (collision_free == false)
    {
        ...
        ball_ee_pose_array = m_RobotMotionController_rob_1->getRandEEPose();
        ...
        //Check is it is collision free
        //if(m_FeasibilityChecker_rob_1->isConfigValid(rand_conf_array))
            collision_free = true;
...
void BiRRTstarMultiRobotPlanner::jointConfigEllipseInitialization()
{

    //Vectors to be used for computation of matrix "M"
    Eigen::VectorXd  a1_rev(m_num_joints_revolute);
    Eigen::VectorXd  id_m1_rev(m_num_joints_revolute);
    Eigen::VectorXd  a1_prism(m_num_joints_prismatic);
    Eigen::VectorXd  id_m1_prism(m_num_joints_prismatic);

    //Current Joint Index
    int joint_idx = 0;
    int joint_idx_rev = 0;
    int joint_idx_prism = 0;
    for (int k = 0 ; k < m_manipulator_chain_rob_1.getNrOfSegments(); k++)
     {
         if(m_manipulator_chain_rob_1.getSegment(k).getJoint().getTypeName() != "None")
         {
             //Collect sum of squares for prismatic and revolute joints
             if(m_manipulator_chain_rob_1.getSegment(k).getJoint().getTypeName() != "RotAxis")
             {
...
//    }
    ...
    //Matrix required to compute matrix "C"
    Eigen::MatrixXd matrix_M_rev (m_num_joints_revolute, m_num_joints_revolute);
    matrix_M_rev = a1_rev*id_m1_rev.transpose();

    Eigen::MatrixXd matrix_M_prism (m_num_joints_prismatic, m_num_joints_prismatic);
    ...
    //Diagonal matrix required to compute matrix "C"
    Eigen::MatrixXd diag_matrix_rev (m_num_joints_revolute, m_num_joints_revolute);
    Eigen::MatrixXd diag_matrix_prism (m_num_joints_prismatic, m_num_joints_prismatic);
...
KDL::JntArray BiRRTstarMultiRobotPlanner::sampleJointConfigfromEllipse_JntArray(string tree_name)
{
...
//    double time_start = m_timer.tv_sec+(m_timer.tv_usec/1000000.0);
    ...
    //Difference in number of revolute joints between current robot and robot with max DOF's of all robots
    int diff_num_rev_joints = 0;
    ...
    else if(tree_name == "GOAL")
    {
        ...
        //Diff in number of revolute joints
        diff_num_rev_joints = m_KDLRobotModel_rob_1->getNumRevoluteJoints() - m_KDLRobotModel_rob_2->getNumRevoluteJoints();
    }
    else
    {
        ROS_ERROR("tree not known!");
    ...
    Eigen::VectorXd  ball_conf_revolute(m_num_joints_revolute + diff_num_rev_joints);
    Eigen::VectorXd  ball_conf_prismatic(m_num_joints_prismatic);

    //Transformation Matrix "L"
    Eigen::MatrixXd transformation_L_rev (m_num_joints_revolute + diff_num_rev_joints, m_num_joints_revolute + diff_num_rev_joints);
    ...
    while (collision_free == false || above_platform == false || inside_environment == false)
    {
        ...
        ball_conf_array = controller->getRandomConf(m_env_size_x, m_env_size_y);

        //Split config vector into config of revolute and prismatic joints
        int joint_idx = 0;
        int joint_idx_rev = 0;
        int joint_idx_prism = 0;
        for (int k = 0 ; k < kinematic_chain.getNrOfSegments(); k++)
         {
             if(kinematic_chain.getSegment(k).getJoint().getTypeName() != "None")
             {
                 if(kinematic_chain.getSegment(k).getJoint().getTypeName() == "RotAxis")
                 {
        ...
        //Artificially extend revolute joint config for lower DOF's robot model
        for(int aj = m_num_joints_revolute ; aj < m_num_joints_revolute + diff_num_rev_joints; aj++)
        {
            ball_conf_revolute[aj] = 0.0;
        }
        ...
        // ------------ START: Computation of the random configuration from ellipse ----------

        //Configuration/Sample (xf in paper)
        Eigen::VectorXd  rand_conf_rev(m_num_joints_revolute + diff_num_rev_joints);
        ...
        vector<double> ee_pose = robot_model->compute_FK(kinematic_chain,rand_conf_array);
        ...
        if(2 <= m_num_joints_prismatic)
        {
            ...
            if( ( rand_conf_array(0) < m_env_size_x[1] && rand_conf_array(0) > m_env_size_x[0] && rand_conf_array(1) < m_env_size_y[1] && rand_conf_array(1) > m_env_size_y[0] ) || (m_env_size_x[0] == 0.0 && m_env_size_x[1] == 0.0 && m_env_size_y[0] == 0.0 && m_env_size_y[1] == 0.0) )
        ...
        if(feasibility_checker->isConfigValid(rand_conf_array))
            collision_free = true;
        else
...
KDL::JntArray BiRRTstarMultiRobotPlanner::sampleJointConfig_JntArray(string tree_name)
{
    //Robot controller, model and Feasibility checker to be used
    boost::shared_ptr<kuka_motion_controller::RobotController> controller;
    boost::shared_ptr<kuka_motion_controller::KDLRobotModel> robot_model;
    ...
    while (collision_free == false || above_platform == false)
    {
        ...
        rand_conf = controller->getRandomConf(m_env_size_x, m_env_size_y);
        ...
        // Get endeffector pose for random config
        vector<double> ee_pose = robot_model->compute_FK(kinematic_chain,rand_conf);
        ...
        //Check is it is collision free
        if(feasibility_checker->isConfigValid(rand_conf))
            collision_free = true;
    }
...
//Sample Configuration
KDL::JntArray BiRRTstarMultiRobotPlanner::sampleJointConfig_JntArray(string tree_name, vector<double> mean_config, double std_dev)
{
    ...
    {
        //Get random configuration
        rand_conf = controller->getRandomConf(mean_config, std_dev, m_env_size_x, m_env_size_y);
...
//Sample Configuration
vector<double> BiRRTstarMultiRobotPlanner::sampleJointConfig_Vector(string tree_name)
{
    ...
    while (collision_free == false || above_platform == false)
    {
        ...
        //Get random configuration
        rand_sample = controller->getRandomConf(m_env_size_x,m_env_size_y);

        vector<double> ee_pose = robot_model->compute_FK(kinematic_chain,rand_sample);
        ...
        //Check is it is collision free
        if(feasibility_checker->isConfigValid(rand_sample))
            collision_free = true;
    }
...
//Sample Configuration
vector<double> BiRRTstarMultiRobotPlanner::sampleJointConfig_Vector(string tree_name, vector<double> mean_config, double std_dev)
{
    ...
    while (collision_free == false || above_platform == false)
    {
        ...
        //Get random configuration
        rand_sample = controller->getRandomConf(mean_config, std_dev, m_env_size_x, m_env_size_y);
...
Node BiRRTstarMultiRobotPlanner::find_nearest_neighbour_control(Rrt_star_tree *tree, vector<double> x_rand)
{
...
Node BiRRTstarMultiRobotPlanner::find_nearest_neighbour_interpolation(Rrt_star_tree *tree, vector<double> x_rand)
{
...
//            vector<double> dist_to_start_tree_root = m_Heuristic.euclidean_joint_space_distance(m_manipulator_chain_rob_1, m_config_start_pose, tree->nodes[n].config);
//            vector<double> dist_to_goal_tree_root = m_Heuristic.euclidean_joint_space_distance(m_manipulator_chain_rob_1, tree->nodes[n].config, m_config_goal_pose);
...
double BiRRTstarMultiRobotPlanner::compute_edge_cost_control(vector<vector<double> > ee_traj)
{
...
//Compute the cost of an edge connecting two nodes
vector<double> BiRRTstarMultiRobotPlanner::compute_edge_cost_interpolation(string tree_name, vector<vector<double> > joint_traj)
{
    //Kinematic chain to be used
    KDL::Chain kinematic_chain;

    //Edge cost weights
    vector<double> edge_cost_weights;

    if(tree_name == "START")
    {
        kinematic_chain = m_manipulator_chain_rob_1;
        edge_cost_weights = m_edge_cost_weights_rob_1;
    }
    else if(tree_name == "GOAL")
    {
        kinematic_chain = m_manipulator_chain_rob_2;
        edge_cost_weights = m_edge_cost_weights_rob_2;
    }
    else
    {
        ROS_ERROR("tree not known!");
        ...
        for (int k = 0 ; k < kinematic_chain.getNrOfSegments(); k++)
         {
             if(kinematic_chain.getSegment(k).getJoint().getTypeName() != "None")
             {
                 ...
                 // -> + weights individual costs by "edge_cost_weights"
                 wp_dist_total += coordinate_dist * edge_cost_weights[joint_idx];
...
//Find the set of vertices in the vicinity of x_new
vector<int> BiRRTstarMultiRobotPlanner::find_near_vertices_control(Rrt_star_tree *tree, Node x_new)
{
...
//Find the set of vertices in the vicinity of x_new
vector<int> BiRRTstarMultiRobotPlanner::find_near_vertices_interpolation(Rrt_star_tree *tree, Node x_new)
{
...
kuka_motion_controller::Status BiRRTstarMultiRobotPlanner::connectNodesControl(Rrt_star_tree *tree, Node near_node, Node end_node, Node &x_new, Edge &e_new)
{
    //Set start configuration and goal pose
    m_RobotMotionController_rob_1->setStartConf(near_node.config);
    m_RobotMotionController_rob_1->set_EE_goal_pose(end_node.ee_pose);

    //Array storing the joint and endeffector trajectory
    vector<vector<double> > joint_trajectory;
    vector<vector<double> > ee_trajectory;
    kuka_motion_controller::Status connector_state = m_RobotMotionController_rob_1->run_VDLS_Control_Connector(500, joint_trajectory,ee_trajectory,0,0);
...
bool BiRRTstarMultiRobotPlanner::connectNodesInterpolation(Rrt_star_tree *tree, Node near_node, Node end_node, int num_points, Node &x_new, Edge &e_new)
{
    ...
    bool valid_interpolation = interpolateConfigurations(tree->name, near_node,end_node, num_points ,joint_trajectory,ee_trajectory);
    ...
    //Compute cost of edge from near_node to x_new
    vector<double> edge_costs = compute_edge_cost_interpolation(tree->name, joint_trajectory);
    ...
    if(tree_idx == true)
        x_new.cost_h.total = m_Heuristic.euclidean_base_distance(x_new.config, m_config_goal_pose);
    else
        x_new.cost_h.total = m_Heuristic.euclidean_base_distance(x_new.config, m_config_start_pose);
...
//Interpolate configurations of near_node and end_node
bool BiRRTstarMultiRobotPlanner::interpolateConfigurations(string tree_name, Node near_node, Node end_node, int num_points ,vector<vector<double> > &joint_traj, vector<vector<double> > &ee_traj)
{
...
//    cout<<endl;
    ...
    for(int inc = 0 ; inc <= num_points ; inc++)
    {
        ...
        vector<double> curr_ee_pose = robot_model->compute_FK(kinematic_chain,c_space_via_point_array);
        ...
        {
            //Compute task error
            vector<double> task_error_interpolation = controller->compute_task_error(c_space_via_point_vector, m_task_frame, m_grasp_transform, m_constraint_vector, m_coordinate_dev);
            //double task_error_interpolation_norm =controller->getVectorNorm(task_error_interpolation);
            bool task_error_within_bounds = controller->is_error_within_bounds(task_error_interpolation);
...
//Choose Parent for x_new minimizing the cost of reaching x_new (given the set of vertices surrounding x_new as potential parents)
bool BiRRTstarMultiRobotPlanner::choose_node_parent_control(Rrt_star_tree *tree, vector<int> near_vertices, Node nn_node, Edge &e_new , Node &x_new)
{
    //-- Initialize --
...
//Choose Parent for x_new minimizing the cost of reaching x_new (given the set of vertices surrounding x_new as potential parents)
bool BiRRTstarMultiRobotPlanner::choose_node_parent_interpolation(Rrt_star_tree *tree, vector<int> near_vertices, Node nn_node, Edge &e_new, Node &x_new, bool show_tree_vis)
{
    ...
    for (int nv = 0; nv < near_vertices.size() ; nv++)
    {
       ...
       if(tree->nodes[near_vertices[nv]].cost_reach.total < x_new.cost_reach.total)
       {
           ...
           if(m_constraint_active == false)
           {
               ...
               {
                    //Only if edge to x_new is valid
                    int last_valid_node_idx = 0; //Not used but required by feasibility_checker->isEdgeValid-Function
                    if(feasibility_checker->isEdgeValid(gen_edge,last_valid_node_idx))
                    {
                        //A parent for the new node has been found
                        parent_found = true;
...
//                        break;
           ...
           else //Motion Constraints Active
           {
               ...
               while(new_sample_reached == false)
               {
                   ...
                   new_sample_reached = stepTowardsRandSample(tree->name, curr_x_near, exp_node_towards_x_new, m_constraint_extend_step_factor);
                   ...
                   if(new_sample_reached == false)
                   {
                       ...
                       //Project sample onto constraint manifold
                       bool projection_succeed = controller->run_config_retraction(exp_node_towards_x_new.config, m_task_frame, m_grasp_transform, m_constraint_vector, m_coordinate_dev, m_max_projection_iter);


                       if(projection_succeed == true && feasibility_checker->isConfigValid(exp_node_towards_x_new.config))
                       {
...
void BiRRTstarMultiRobotPlanner::rewireTreeControl(Rrt_star_tree *tree, vector<int> near_vertices, Node x_new, bool show_tree_vis)
{
...
//Rewire Nodes of the Tree, i.e. checking if some if the nodes can be reached by a lower cost path
void BiRRTstarMultiRobotPlanner::rewireTreeInterpolation(Rrt_star_tree *tree, vector<int> near_vertices, Node x_new, bool show_tree_vis)
{
                ...
                if(m_constraint_active == false)
                {
                    ...
                    if(gen_node.cost_reach.total < tree->nodes[near_vertices[nn_num]].cost_reach.total)
                    {
                        ...
                        if(feasibility_checker->isEdgeValid(gen_edge,last_valid_node_idx))
                        {
                            ...
                            // -> Update Heuristic value (now slightly different from previous one due to controller reach tolerance)
                            if(tree->name == "START")
                                tree->nodes[near_vertices[nn_num]].cost_h.total = m_Heuristic.euclidean_base_distance(gen_node.config, m_config_goal_pose);
                            else
                                tree->nodes[near_vertices[nn_num]].cost_h.total = m_Heuristic.euclidean_base_distance(gen_node.config, m_config_start_pose);
...
//                    int total_num_iter_while_loop = 0;
                    ...
                    while(new_sample_reached == false)
                    {
                        ...
                        new_sample_reached = stepTowardsRandSample(tree->name, curr_x_new, exp_node_towards_x_near, m_constraint_extend_step_factor);
                        ...
                        if(new_sample_reached == false)
                        {
                            ...
                            //Project sample onto constraint manifold
                            bool projection_succeed = controller->run_config_retraction(exp_node_towards_x_near.config, m_task_frame, m_grasp_transform, m_constraint_vector, m_coordinate_dev, m_max_projection_iter);
                            ...
                            if(projection_succeed == true && feasibility_checker->isConfigValid(exp_node_towards_x_near.config))
                            {
...
void BiRRTstarMultiRobotPlanner::recursiveNodeCostUpdate(Rrt_star_tree *tree, Node tree_vertex, vector<double> cost_reduction, bool show_tree_vis)
{ 
...
void BiRRTstarMultiRobotPlanner::setParameterizedTaskFrame(vector<int> cv, vector<pair<double, double> > coordinate_dev, bool task_pos_global, bool task_orient_global)
{
    ...
    //Convert start configuration into KDL Joint Array
    KDL::JntArray start_configuration = m_RobotMotionController_rob_1->Vector_to_JntArray(m_config_start_pose);
...
void BiRRTstarMultiRobotPlanner::setFixedTaskFrame(vector<int> cv, vector<pair<double,double> > coordinate_dev, KDL::Frame task_frame)
{
    //Check dimension of constraint vector
    if(cv.size() > 6)
        ROS_ERROR("Dimension of constraint vector larger than task space dimension!!!");
    ...
    KDL::Frame start_ee_pose = m_KDLRobotModel_rob_1->compute_FK_frame(m_manipulator_chain_rob_1, start_configuration);
...
void BiRRTstarMultiRobotPlanner::setEdgeCostWeights(vector<double> ecw, int robot_id)
{
    if (robot_id == 1)
    {
        if(ecw.size() < m_KDLRobotModel_rob_1->getNumJoints())
        ...
        for(int j = 0 ; j < m_KDLRobotModel_rob_1->getNumJoints() ; j++)
        {
            m_edge_cost_weights_rob_1[j] = ecw[j];
        }
    }
    else if(robot_id == 2)
    {
        if(ecw.size() < m_KDLRobotModel_rob_2->getNumJoints())
            ROS_ERROR("Dimension of edge cost weights vector is smaller than the number of Joints of the Kinematic Chain");

        //Set Edge Cost Weights
        for(int j = 0 ; j < m_KDLRobotModel_rob_2->getNumJoints() ; j++)
        {
            m_edge_cost_weights_rob_2[j] = ecw[j];
        }
    }
    else
        ROS_ERROR("Set edge cost weights either for robot 1 or 2!");

}


////Function to set the configuration projection error threshold (used in constraint satisfaction function)
//void BiRRTstarMultiRobotPlanner::setConfigProjectionErrorThreshold(double threshold)
//{
...
bool BiRRTstarMultiRobotPlanner::stepTowardsRandSample(string tree_name, Node nn_node, Node &x_rand, double extend_step_factor)
{
    //Kinematic chain to be used
    KDL::Chain kinematic_chain;
...
//        KDL::JntArray ext_configuration = m_RobotMotionController_rob_1->Vector_to_JntArray(extend_config);
//        x_rand.ee_pose = m_KDLRobotModel_rob_1->compute_FK(kinematic_chain,ext_configuration);
...
// ------------------------------ Tree Visualization -----------------------------------------------------
void BiRRTstarMultiRobotPlanner::add_tree_edge_vis(string tree_name, Edge new_edge)
{
...
void BiRRTstarMultiRobotPlanner::remove_tree_edge_vis(string tree_name, Edge old_edge)
{
...
void BiRRTstarMultiRobotPlanner::add_tree_node_vis(string tree_name, Node new_node)
{
...
void BiRRTstarMultiRobotPlanner::add_tree_node_vis(string tree_name, Node new_node, vector<double> color_rgb)
{
    if(color_rgb.size() != 3)
...
void BiRRTstarMultiRobotPlanner::showCurrentSolutionPath(string connected_tree_name, Node node_tree_B, Node node_tree_A)
{
...
void BiRRTstarMultiRobotPlanner::computeFinalSolutionPathTrajectories()
{
    ...
    for (int spe = 0 ; spe < solution_path_edges_start_tree.size() ; spe++)
    {
        for(int element = 0 ; element < solution_path_edges_start_tree[spe].joint_trajectory.size()-1 ; element++)
        {
            ...
            m_result_joint_trajectory_rob_1.push_back(solution_path_edges_start_tree[spe].joint_trajectory[element]);
            m_result_ee_trajectory_rob_1.push_back(solution_path_edges_start_tree[spe].ee_trajectory[element]);
    ...
    for (int spe = 0 ; spe < solution_path_edges_goal_tree.size() ; spe++)
    {
        ...
        {
            //Note: Edge joint and ee trajectories have same number of elements
            m_result_joint_trajectory_rob_2.push_back(solution_path_edges_goal_tree[spe].joint_trajectory[element]);
            m_result_ee_trajectory_rob_2.push_back(solution_path_edges_goal_tree[spe].ee_trajectory[element]);
        }

        //Enter last point of last goal tree edge (i.e. the final point)
        if(spe == solution_path_edges_goal_tree.size()-1)
        {
            m_result_joint_trajectory_rob_2.push_back(solution_path_edges_goal_tree[spe].joint_trajectory[0]);
            m_result_ee_trajectory_rob_2.push_back(solution_path_edges_goal_tree[spe].ee_trajectory[0]);
        }
    }
    ...
    //Write Planning results to file (joint and ee trajectory for robot 1 and 2)
    m_KDLRobotModel_rob_1->writeTrajectoryToFile(m_result_joint_trajectory_rob_1,m_file_path_joint_trajectory_rob_1);
    m_KDLRobotModel_rob_1->writeTrajectoryToFile(m_result_joint_trajectory_rob_2,m_file_path_joint_trajectory_rob_2);
    m_KDLRobotModel_rob_1->writeTrajectoryToFile(m_result_ee_trajectory_rob_1,m_file_path_ee_trajectory_rob_1);
    m_KDLRobotModel_rob_1->writeTrajectoryToFile(m_result_ee_trajectory_rob_2,m_file_path_ee_trajectory_rob_2);
...
//Write Planner Statistics to File
void BiRRTstarMultiRobotPlanner::writePlannerStatistics(char *statistics_file, char *cost_evolution_file)
{
    //+++++++++ Write planner statistics to file ++++++++
...
void BiRRTstarMultiRobotPlanner::writeStartGoalConfig(char *start_goal_config_file, vector<double> start_config, vector<double> goal_config)
{
...
bool BiRRTstarMultiRobotPlanner::readStartGoalConfig(char *start_goal_config_file, vector<double> &start_config, vector<double> &goal_config)
{
...
void BiRRTstarMultiRobotPlanner::attachObject(moveit_msgs::AttachedCollisionObject attached_object, planning_world::PlanningWorldBuilder *pwb)
{
    ...
    pwb->attachObjecttoEndeffector(attached_object,m_ee_start_pose);

}

//DeAttach Object to the End-effector
void BiRRTstarMultiRobotPlanner::detachObject(moveit_msgs::AttachedCollisionObject attached_object, planning_world::PlanningWorldBuilder *pwb)
{
    //Dettach the given object "attached_object" from the end-effector in the goal pose m_goal_tree.nodes[0].ee_pose
    pwb->detachObjectfromEndeffector(attached_object,m_ee_goal_pose);
}
...
//void BiRRTstarMultiRobotPlanner::basic_solution_path_smoothing(vector<vector<double> > raw_joint_trajectory)
//{
...
//    m_KDLRobotModel_rob_1->getJointRangeData(m_manipulator_chain_rob_1 , q_min, q_max, q_opt_pos);
...
void BiRRTstarMultiRobotPlanner::no_two_parents_check(Rrt_star_tree *tree)
{
...
void BiRRTstarMultiRobotPlanner::cost_consistency_check(Rrt_star_tree *tree)
{
    for (int i = 0 ; i < tree->nodes.size() ; i++)
    {
