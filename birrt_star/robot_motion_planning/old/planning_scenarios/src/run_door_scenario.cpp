...
{
    //Init Node
    ros::init(argc, argv, "run_door_scenario_node");
    ...
    //Insert Door Scenario
    // -> TODO: Create door world
    //Set scene name manually
    world_builder.setSceneName("door_scene");

    //Path to the file containing the start and goal config for the scenario
    char* file_path_start_goal_config;
    string folder_path = terminal_configs_path + "/Start_Goal_Configurations/" + PLANNING_GROUP + "_door_start_goal_config.txt";
...
//    //  0 -> unconstraint
    ...
    constraint_vector[1] = 1.0; //Y translation
    constraint_vector[2] = 1.0; //Z translation
    ...
    permitted_coordinate_dev[0].first = -0.03;    //negative X deviation [m]
    permitted_coordinate_dev[0].second = 0.03;   //positive X deviation
    permitted_coordinate_dev[1].first = -0.03;    //negative Y deviation
    permitted_coordinate_dev[1].second = 0.03;   //positive Y deviation
    permitted_coordinate_dev[2].first = -0.03;    //negative Z deviation
    permitted_coordinate_dev[2].second = 0.03;   //positive Z deviation
    permitted_coordinate_dev[3].first = -0.087;    //negative Xrot deviation [rad]
    permitted_coordinate_dev[3].second = 0.087;   //positive Xrot deviation
    permitted_coordinate_dev[4].first = -0.087;    //negative Yrot deviation
    permitted_coordinate_dev[4].second = 0.087;   //positive Yrot deviation
    permitted_coordinate_dev[5].first = -1.6;    //negative Zrot deviation
    permitted_coordinate_dev[5].second = 0.087;   //positive Zrot deviation

    //Set the fixed task frame
    KDL::Vector pos_hinge_frame(0.0,0.0,0.0);
    KDL::Rotation orient_hinge_frame = KDL::Rotation::Identity();
    KDL::Frame hinge_frame(orient_hinge_frame,pos_hinge_frame);
...
//    // bool task_orient_global -> indicates whether task frame orientation is expressed w.r.t near node ee orientation or always w.r.t start frame ee orientation
    ...
    if (search_direction == uni)
    {
        ...
        //Activate the constraint
        uni_planner.setFixedTaskFrame(constraint_vector, permitted_coordinate_dev, hinge_frame);
    ...
    else if (search_direction == bi)
    {
        ...
        //Activate the constraint
        bi_planner.setFixedTaskFrame(constraint_vector, permitted_coordinate_dev, hinge_frame);
