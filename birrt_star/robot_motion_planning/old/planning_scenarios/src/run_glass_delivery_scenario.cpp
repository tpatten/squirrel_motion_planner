/*
 *run_glass_delivery_scenario.cpp
 *
...
{
    //Init Node
    ros::init(argc, argv, "run_glass_delivery_scenario_node");
	...
	// -------------------- Attached Objects ----------------------------
    moveit_msgs::AttachedCollisionObject glass;
    moveit_msgs::AttachedCollisionObject glass_goal;
    if(PLANNING_GROUP == "omnirob_lbr_sdh")
    {
        ...
        glass = world_builder.insertManipulableGlass("glass", obj_pos_start, glass_dim);
        ...
        glass_goal = world_builder.insertManipulableGlass("glass_goal", obj_pos_goal, glass_dim);
    }
    if(PLANNING_GROUP == "robotino_robot")
    {
        //Not implemented yet
    ...
    // Init planner with start/goal pose + Set end-effector constraints + Set Edge Cost Weightss
    if (search_direction == uni)
    {
        ...
        if(PLANNING_GROUP == "omnirob_lbr_sdh")
        {
            ...
            //Attach the cart to the endeffector being in the start pose before planning (in order to consider the glass in collision checking)
    ...
    // -------------------- Detach Cart after Planning ----------------------------
	
	if (search_direction == uni)
    {
        //Detach the object after planning
        uni_planner.detachObject(glass);
        world_builder.deleteCollisionObject(glass);
    }
    else if (search_direction == bi)
    {
        //Detach the object after planning
        bi_planner.detachObject(glass);
        world_builder.deleteCollisionObject(glass);
