...
{
    public:
    RRTstarPlanner(string planning_group);
    ~RRTstarPlanner();
    ...
    bool init_planner_real_robot_goal_pose(const Eigen::Affine3d& goal, const vector<int> constraint_vec_goal_pose, const vector<pair<double,double> > target_coordinate_dev, const string planner_type);
    ...
    //Generate configurations for ee poses
    ...
    //Function to set constraint vector
    ...
    //Run RRT* Planner (given max. iterations)
    ...
    //Unidirectional planner type
    string m_planner_type;

    //Flag indicating that the goal node is in the near vertices set
    bool m_goal_node_near;
    ...
    visualization_msgs::MarkerArray m_tree_add_edge_marker_array_msg; //Vector of Line Strips / Edges

    // Marker for nodes (SPHERE_LIST)
    visualization_msgs::Marker m_tree_add_nodes_marker; //add nodes to tree visualization

    //Tree edges and node Publisher
    ros::Publisher m_tree_edge_pub;
    ros::Publisher m_tree_node_pub;
    ...
    void remove_tree_edge_vis(Edge old_edge);
