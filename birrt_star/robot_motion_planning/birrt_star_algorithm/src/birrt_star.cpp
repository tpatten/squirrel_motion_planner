...
//  m_FeasibilityChecker = boost::shared_ptr<state_feasibility_checker::FeasibilityChecker>(
//      new state_feasibility_checker::FeasibilityChecker(robot_description_robot, m_planning_group, m_ns_prefix_robot));
//
//
  ...
  m_RobotMotionController = boost::shared_ptr<kuka_motion_controller::RobotController>(
      new kuka_motion_controller::RobotController(robot_description_robot, m_planning_group, m_ns_prefix_robot));
...
bool BiRRTstarPlanner::init_planner(vector<double> start_conf, vector<double> goal_conf, int search_space, bool check_self_collision, bool check_map_collision)
{
  ...
  checkSelfCollision = check_self_collision;
  checkMapCollision = check_map_collision;
  ...
  bool start_conf_valid = isConfigValid(start_configuration, checkSelfCollision, checkMapCollision);
  bool goal_conf_valid = isConfigValid(goal_configuration, checkSelfCollision, checkMapCollision);
...
//bool BiRRTstarPlanner::init_planner_real_robot_goal_pose(const Eigen::Affine3d& goal, const vector<int> constraint_vec_goal_pose,
...
//
//    //Initialize Planner
//    bool init_ok = init_planner(start_conf, goal_conf_base, 1);
...
//      listener.lookupTransform("/map", m_ns_prefix_robot + "lbr_0_link", ros::Time(0), transform_map_to_arm_base);
//    }
//    catch (tf::TransformException ex)
//    {
//      ROS_ERROR("%s", ex.what());
...
//    bool init_ok = init_planner(start_conf, goal_pose_endeffector, constraint_vec_goal_pose, target_coordinate_dev, 1);
//    //Remark: Fails is start or goal config (start_conf_base,goal_conf_base) is invalid
//
//    return init_ok;
//  }
...
//}
//
//bool BiRRTstarPlanner::init_planner_real_robot_goal_config(const vector<double> goal, const string planner_type)
//{
                                   ...
                                   int planner_run_number)
{
  ...
  string scenario_name = "scenario"; //m_planning_world->getSceneName();
      ...
      && ((flag_iter_or_time == 0 && m_executed_planner_iter < m_max_planner_iter) || (flag_iter_or_time == 1 && m_executed_planner_time < m_max_planner_time)))
  {
    ...
    if (search_space == 0)
    {
      ...
      bool tree_extend_NN = isEdgeValid(e_new, checkSelfCollision, checkMapCollision);
...
void BiRRTstarPlanner::setOctree(const octomap::OcTree* octree)
{
  collisionChecker->setOcTree(octree);
}

//Finds and returns 8D robot pose from a requested end effector poes
bool BiRRTstarPlanner::getFullPoseFromEEPose(const vector<double> &endEffectorPose, const vector<pair<double, double> > &endEffectorDeviations,
                                             const vector<double> &poseInit, vector<double> &poseSolution)
{
  ...
  goalEEPoseQuatOrient[0] = endEffectorPose[0]; //x
  goalEEPoseQuatOrient[1] = endEffectorPose[1]; //y
  goalEEPoseQuatOrient[2] = endEffectorPose[2]; //z
  goalEEPoseQuatOrient[3] = sx * cy * cz - cx * sy * sz;  //quat_x
  goalEEPoseQuatOrient[4] = cx * sy * cz + sx * cy * sz;  //quat_y
  goalEEPoseQuatOrient[5] = cx * cy * sz - sx * sy * cz;  //quat_z
  goalEEPoseQuatOrient[6] = cx * cy * cz + sx * sy * sz;  //quat_w
...
void BiRRTstarPlanner::setPlanningSceneInfo(vector<double> size_x, vector<double> size_y, string scene_name)
{
                                                ...
                                                bool show_motion)
{
  ...
  while (collision_free == false)
  {
    ...
    if (connector_state == kuka_motion_controller::REACHED)
    {
      ...
      if (isConfigValid(ik_solution, checkSelfCollision, checkMapCollision))
      {
        collision_free = true;
      }
      else
...
bool BiRRTstarPlanner::expandTree(Rrt_star_tree *tree, Node nn_node, Node x_rand, Node &x_new, Edge &e_new, bool show_tree_vis)
{
  ...
  if (m_constraint_active)
  {
    ...
    while (rand_sample_reached == false)
    {
      ...
      if (rand_sample_reached == false)
      {
        ...
        if (projection_succeed == true && isConfigValid(exp_node_towards_rand_node.config, checkSelfCollision, checkMapCollision))
        {
          ...
          if (connection_result == true && isEdgeValid(gen_edge, checkSelfCollision, checkMapCollision))
          {
            //Modify node data
  ...
  else //Unconstraint motion planning
  {
    ...
    if (m_single_extend_step == true)
    {
      ...
      tree_extend_NN = isEdgeValid(e_new, checkSelfCollision, checkMapCollision);
    ...
    else
    {
      ...
      isEdgeValid(e_new, index_last_valid_node, checkSelfCollision, checkMapCollision);
...
void BiRRTstarPlanner::connectGraphsControl(Rrt_star_tree *tree, Node x_connect, Node x_new, bool show_tree_vis)
{
  ...
  if (connector_result == kuka_motion_controller::REACHED)
  {
    ...
    {
      if (isEdgeValid(gen_edge, checkSelfCollision, checkMapCollision))
      {
      ...
      else
      {
        ...
        for (int i = 1; i < gen_edge.ee_trajectory.size() - 1; i += extend_step_width)
        {
          ...
          {
            if (isEdgeValid(ext_edge, checkSelfCollision, checkMapCollision))
            {
              //Set Node ID
...
//    double time_start = m_timer.tv_sec+(m_timer.tv_usec/1000000.0);
  ...
  if (m_constraint_active == false)
  {
    ...
    if (solution_cost[0] < cost_solution_path[0])
    {
      ...
      if (isEdgeValid(gen_edge, index_last_valid_node, checkSelfCollision, checkMapCollision))
      {
        //Set solution path cost (will be probably improved by on of the near nodes, see code below)
      ...
      for (int nv = 0; nv < near_nodes.size(); nv++)
      {
        ...
        if (tree->nodes[near_nodes[nv]].cost_reach.total < x_new.cost_reach.total)
        {
          ...
          if (solution_cost[0] < cost_solution_path[0])
          {
            ...
            int index_last_valid_node = 0;
            //Only if edge to x_new is valid
            if (isEdgeValid(ext_edge, index_last_valid_node, checkSelfCollision, checkMapCollision))
            {
//                            cout<<tree->name<<endl;
  ...
  else
  {
    ...
    while (rand_sample_reached == false)
    {
      ...
      if (rand_sample_reached == false)
      {
        ...
        if (projection_succeed == true && isConfigValid(exp_node_towards_tree_A.config, checkSelfCollision, checkMapCollision))
        {
    ...
    for (int k = 0; k < m_manipulator_chain.getNrOfSegments(); k++)
    {
      if (m_manipulator_chain.getSegment(k).getJoint().getTypeName() != "None")
      {
        ...
        if (m_manipulator_chain.getSegment(k).getJoint().getTypeName() == "RotAxis")
          ...
          //Collect distance between current and next waypoint coordinate (only prismatic joints)
    ...
    if (tree->nodes[near_vertices[nv]].cost_reach.total < x_new.cost_reach.total)
    {
      ...
      if (m_constraint_active == false)
      {
        ...
        if (gen_node.cost_reach.total <= x_new.cost_reach.total)
        {
          ...
          if (isEdgeValid(gen_edge, last_valid_node_idx, checkSelfCollision, checkMapCollision))
          {
            //A parent for the new node has been found
            parent_found = true;
      ...
      else //Motion Constraints Active
      {
        ...
        while (new_sample_reached == false)
        {
          ...
          if (new_sample_reached == false)
          {
            ...
            if (projection_succeed == true && isConfigValid(exp_node_towards_x_new.config, checkSelfCollision, checkMapCollision))
            {
    ...
    if (near_vertices[nn_num] != x_new.parent_id)
    {
      ...
      if (connector_result == kuka_motion_controller::REACHED)
      {
        ...
        if (gen_node.cost_reach.total < tree->nodes[near_vertices[nn_num]].cost_reach.total)
        {
          ...
          tree->nodes[tree->nodes[near_vertices[nn_num]].parent_id].outgoing_edges.erase(
              tree->nodes[tree->nodes[near_vertices[nn_num]].parent_id].outgoing_edges.begin() + out_edge_erase_index);
      ...
      else  //Constraint Active == TRUE
      {
        ...
        while (new_sample_reached == false)
        {
          ...
          if (new_sample_reached == false)
          {
            ...
            if (projection_succeed == true && isConfigValid(exp_node_towards_x_near.config, checkSelfCollision, checkMapCollision))
            {
...
void BiRRTstarPlanner::writePlannerStatistics(char *statistics_file, char *cost_evolution_file)
{
 ...
 << "cost_theoretical_solution_revolute" << " "
      << m_cost_theoretical_solution_path[1] << endl\
...
void BiRRTstarPlanner::transform_sample_to_base_link_frame(KDL::JntArray& sample_conf)
{
  ...
  if (m_planning_frame == "/map" && m_num_joints_prismatic == 2 && m_num_joints_revolute != 0) //-> m_num_joints_prismatic == 2 means robot base is involved in planning
  {
    ...
    if (transform_map_to_base_available)
    {
      ...
      transform_base_link_to_sample.mult(transform_map_to_base.inverse(), transform_map_to_sample);
  ...
  for (int i = 0; i < tree->nodes.size(); i++)
  {
    for (int e = 0; e < tree->nodes[i].outgoing_edges.size(); e++)
    {
      if (tree->nodes[tree->nodes[i].outgoing_edges[e].child_node_id].cost_reach.total < tree->nodes[i].cost_reach.total)
      {
        ...
        cout << "Cost of child node: " << tree->nodes[tree->nodes[i].outgoing_edges[e].child_node_id].node_id << " smaller than cost of parent node: "
            << tree->nodes[tree->nodes[i].outgoing_edges[e].child_node_id].parent_id << " " << tree->nodes[i].node_id << endl;
        cout << "Parent of child node: " << tree->nodes[tree->nodes[i].outgoing_edges[e].child_node_id].parent_id << endl;
        cout << "Cost of child: " << tree->nodes[tree->nodes[i].outgoing_edges[e].child_node_id].cost_reach.total << " smaller than cost of parent: "
            << tree->nodes[i].cost_reach.total << endl;
...
bool BiRRTstarPlanner::isEdgeValid(const Edge &tree_edge, bool check_self_collision, bool check_map_collision)
{
  for (int edp = 0; edp < tree_edge.joint_trajectory.size(); edp++)
  {
    std::vector<double> config(8);
    for (int i = 0; i < 8; ++i)
      config[i] = tree_edge.joint_trajectory[edp][i];

    if (collisionChecker->isInCollision(config, check_self_collision, check_map_collision))
      return false;
  }

  return true;
}
bool BiRRTstarPlanner::isEdgeValid(const Edge &tree_edge, int &last_valid_node_idx, bool check_self_collision, bool check_map_collision)
{
...
bool BiRRTstarPlanner::isConfigValid(const vector<double> &config, bool check_self_collision, bool check_map_collision)
{
  return !collisionChecker->isInCollision(config, check_self_collision, check_map_collision);
}
bool BiRRTstarPlanner::isConfigValid(const KDL::JntArray& config, bool check_self_collision, bool check_map_collision)
{
  std::vector<double> configStd(8);
  for (int i = 0; i < 8; ++i)
